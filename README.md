...
The rofi control center is available with `$mod+u`. It links the app launcher, window switcher, scratchpad terminal, wallpaper gallery, keybinding help, notes, media controls, radio, monitor controls, keyboard layout, audio, network, Bluetooth, clipboard, notification, screenshot, Pomodoro timer, power, lock, and i3 reload actions. Use `$mod+Shift+/` to search the cheat sheet directly, `$mod+Shift+Return` to toggle the floating scratchpad kitty terminal, `$mod+Shift+g` to open the wallpaper gallery, `$mod+o` for Markdown notes, `$mod+m` for media controls, and `$mod+Shift+m` for radio. New scratchpad terminals start in the focused terminal's working directory when that can be detected.

The wallpaper gallery searches `~/.config/wallpapers` recursively for common image formats and shows them as preview tiles. The persisted wallpaper opens preselected, and pressing Enter applies the highlighted image and saves its path in `~/.config/i3/wallpaper` so startup and monitor-layout changes restore it. Set `I3_WALLPAPER_DIR="$HOME/Pictures/Wallpapers"` in `~/.config/i3/theme.env` to use another folder.
